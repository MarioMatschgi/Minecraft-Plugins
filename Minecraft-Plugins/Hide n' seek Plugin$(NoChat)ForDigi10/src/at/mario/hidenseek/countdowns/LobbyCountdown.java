			...
			public void run() {
				...
				for (int i = 0; i < players.size(); i++) {
					...
					if (player.hasPermission("hs.start") || player.isOp()) {
						...
						if (Main.getInstance().getConfig().getBoolean("Config.lobbyitems.startItem.enchantedGlow")) {
	...
	public void idle(String arenaName) {
		...
		idleIDs.put(arenaName, Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.getPlugin(), new Runnable() {
			...
			public void run() {
				...
				if (missingPlayers > 0)
					Main.sendToArenaOnly(arenaName, mm.getMessages().getString("Messages.missingPlayersToStart").replace("%prefix%", mm.getMessages().getString("Messages.prefix")).
