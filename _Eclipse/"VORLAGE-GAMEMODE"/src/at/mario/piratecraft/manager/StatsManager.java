package at.mario.piratecraft.manager;
...
public class StatsManager {
	...
	public static void setStats(Player p, int wins, int loses, int gamesPlayed, int spawnsDestroyed) {
		...
		getStats().set("Stats." + p.getName() + ".spawnsDestroyed", spawnsDestroyed);
	...
	public static void sendStats(String statsOf, Player p) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			int spawnsDestroyed = getStats().getInt("Stats." + p.getName() + ".spawnsDestroyed");
			...
			p.sendMessage(string.replace("%prefix%", mm.getMessages().getString("Messages.prefix")).replace("%statsOf%", statsOf).replace("%wins%", wins+"").replace("%loses%", loses+"").replace("%gamesplayed%", gamesPlayed+"").
					replace("%spawnsdestroyed%", spawnsDestroyed+""));
		}
	}
}
