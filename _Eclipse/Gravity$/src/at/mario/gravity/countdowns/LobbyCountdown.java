		...
		taskIDs.put(arenaName, Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.getPlugin(), new Runnable() {
			...
			public void run() {
				...
				if (list.contains(seconds.get(arenaName))) {
					ChatUtil.sendToArenaOnly(arenaName, mm.getMessages().getString("Messages.countdownBroadcast").replace("%prefix%", mm.getMessages().getString("Messages.prefix")).replace("%seconds%", seconds.get(arenaName)+""));
				}
