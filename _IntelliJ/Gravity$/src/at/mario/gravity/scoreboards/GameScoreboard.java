...
public class GameScoreboard {
	...
	public static void setScoreboard(String arenaName, Player p) {
		...
		if (IngameState.ArenaPlayerLevels.containsKey(arenaName) && IngameState.ArenaPlayerLevels.get(arenaName).containsKey(p) && IngameState.ArenaPlayerLevels.get(arenaName) != null && 
			...
			/*
			if (votedMaps.get(IngameState.ArenaPlayerLevels.get(arenaName).get(p) - 1) != null) {
				map = votedMaps.get(IngameState.ArenaPlayerLevels.get(arenaName).get(p) - 1).getName();
			}
			*/

			if (IngameState.ArenaUsedMaps.get(arenaName).get(IngameState.ArenaPlayerLevels.get(arenaName).get(p) - 1) != null) {
				map = IngameState.ArenaUsedMaps.get(arenaName).get(IngameState.ArenaPlayerLevels.get(arenaName).get(p) - 1).getName();
			}
		}
		...
		for (int i = 0; i < list.size(); i++) {
			...
			obj.getScore(string.replace("%money%", money).replace("%arena%", arenaName).replace("%currentplayers%", currentPlayers+"").replace("%seconds%", String.format("%02d", seconds)).replace("%minutes%", minutes+"").
					...
					replace("%stagenumber%", IngameState.ArenaPlayerLevels.get(arenaName).get(p)+"").
