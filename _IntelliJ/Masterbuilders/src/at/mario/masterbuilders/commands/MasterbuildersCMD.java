package at.mario.masterbuilders.commands;
...
import at.mario.masterbuilders.inventories.SetupInventory;
import at.mario.masterbuilders.manager.PackageSender;
...
public class MasterbuildersCMD implements CommandExecutor {
				...
				if (args[0].equalsIgnoreCase("arena")) {
					if (args.length == 1) {
						...
						if (args[1].equalsIgnoreCase("list")) {
							if (p.hasPermission("masterbuilders.admin") || p.hasPermission("masterbuilders.arena.list") || p.isOp()) {
							...
							if (p.hasPermission("masterbuilders.admin") || p.hasPermission("masterbuilders.arena.create") || p.isOp()) {
								if (args.length == 3) {
							...
							if (p.hasPermission("masterbuilders.admin") || p.hasPermission("masterbuilders.arena.remove") || p.isOp()) {
								if (args.length == 3) {
									if (dm.getData().contains("Data.arenas." + args[2])) {
							...
							if (p.hasPermission("masterbuilders.admin") || p.hasPermission("masterbuilders.arena.setup") || p.isOp()) {
								if (args.length == 3) {
									if (dm.getData().contains("Data.arenas." + args[2])) {
					...
					if (p.hasPermission("masterbuilders.admin") || p.hasPermission("masterbuilders.joinme") || p.isOp()) {
					...
					if (p.hasPermission("masterbuilders.admin") || p.hasPermission("masterbuilders.setmainlobby") || p.isOp()) {
	...
	public void ArenaCMDInfo(Player p) {
		p.sendMessage("--------§3Master§bBuilders§f--(/§amasterbuilders arena§f)----------------");
		p.sendMessage("§6/masterbuilders arena §f| §eThe main subcommand");
		p.sendMessage(" ");
		p.sendMessage("§6/masterbuilders arena §f| §eShows you this info");
		p.sendMessage("§6/masterbuilders arena list §f| §eLists all arenas");
		p.sendMessage("§6/masterbuilders arena create {arena} §f| §eCreates an arena {arena}");
		p.sendMessage("§6/masterbuilders arena remove {arena} §f| §eRemoves an arena {arena}");
		p.sendMessage("§6/masterbuilders arena setup {arena} §f| §eOpens the setup menue for the arena {arena}");
		p.sendMessage("§f------------------------------------------------------");
	}

	public static void CMDInfo(Player p) {
		p.sendMessage("--------§3Master§bBuilders§f------------------------------------");
		p.sendMessage("§6/masterbuilders §f| §eThe main command");
		p.sendMessage("§6Aliasses§f: §emb§f, §embr§f, §emasterbuildersr");
		p.sendMessage(" ");
		p.sendMessage("§6/masterbuilders help §f| §eShows you this info");
		p.sendMessage("§6/masterbuilders arena §f| §eShows you the info of the \"/arena\" subcommand");
		p.sendMessage("§6/masterbuilders setmainlobby §f| §eSets the mainlobby spawn");
		p.sendMessage("§f------------------------------------------------------");
	}
