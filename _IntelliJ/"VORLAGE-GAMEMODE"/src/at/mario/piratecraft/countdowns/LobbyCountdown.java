...
import at.mario.piratecraft.utils.ChatUtil;
		...
		taskIDs.put(arenaName, Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.getPlugin(), new Runnable() {
			...
			public void run() {
				...
				for (int i = 0; i < players.size(); i++) {
					...
					if (player.hasPermission("piratecraft.start") || player.isOp()) {
						ItemStack start = new ItemStack(Material.valueOf(Main.getInstance().getConfig().getString("Config.lobbyitems.startItem.material")), 
								Main.getInstance().getConfig().getInt("Config.lobbyitems.startItem.amount"), (short) Main.getInstance().getConfig().getInt("Config.lobbyitems.startItem.damage"));
				...
				if (list.contains(seconds.get(arenaName))) {
					ChatUtil.sendToArenaOnly(arenaName, mm.getMessages().getString("Messages.countdownBroadcast").replace("%prefix%", mm.getMessages().getString("Messages.prefix")).replace("%seconds%", 
							seconds.get(arenaName)+""));
				}
	...
	public void idle(String arenaName) {
		...
		idleIDs.put(arenaName, Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.getPlugin(), new Runnable() {
			...
			public void run() {
				...
				ChatUtil.sendToArenaOnly(arenaName, mm.getMessages().getString("Messages.missingPlayersToStart").replace("%prefix%", mm.getMessages().getString("Messages.prefix")).
						replace("%missingplayers%", missingPlayers+""));
			}
